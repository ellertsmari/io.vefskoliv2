...
export const TeacherHomePage = ({ extendedGuides, modules }: TeacherHomePageProps) => {
  ...
  const handleCreateGuide = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert("Could not create a new guide.");
        setCreatingGuide(false);
        return;
      }
      const { id } = await response.json();
      window.location.href = `/LMS/edit-guides/${id}`;
    } catch (error) {
  ...
  return (
    <ActionGrid>
      ...
      <ActionCard
        ...
        style={creatingGuide ? { pointerEvents: "none", opacity: 0.6 } : undefined}
      >
        <ActionIcon>
          <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5}>
        ...
        <ActionTitle>{creatingGuide ? "CREATING…" : "CREATE NEW GUIDE"}</ActionTitle>
      </ActionCard>
      ...
      <ActionCard onClick={() => window.location.href = '/LMS/groups'}>
        <ActionIcon>
          <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5}>
